...
- **Orval generated types can be overwritten**: Files under `packages/api/src/generated/` are regenerated by `pnpm generate:api`. If the backend adds new fields (e.g. `moderationStatus`, `visibilityStatus`) but the swagger hasn't been regenerated yet, the generated TypeScript types will be stale. Fix by either running `pnpm generate:api` against the running backend, or manually adding the missing fields to the generated file and committing. Hand-written hooks for new endpoints belong in `packages/api/src/<feature>-hooks.ts` (not in `generated/`), and must be exported from `packages/api/src/index.ts`.
